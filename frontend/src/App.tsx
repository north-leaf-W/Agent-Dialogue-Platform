...
const App: React.FC = () => {
    ...
    const checkApiKeyStatus = async () => {
        try {
            const response = await fetch('http://localhost:8000/api/key-status');
            const data = await response.json();
            
            if (!data.has_key) {
                // 如果后端没有API Key，但本地有，则同步到后端
                const savedApiKey = localStorage.getItem('dashscope_api_key');
                if (savedApiKey) {
                    await syncApiKeyToBackend(savedApiKey);
                }
            }
        } catch (error) {
    ...
    // 同步API Key到后端
    const syncApiKeyToBackend = async (apiKey: string) => {
        try {
            await fetch('http://localhost:8000/api/set-key', {
                method: 'POST',
                headers: {
            ...
            console.log('API Key已同步到后端');
        } catch (error) {
            console.error('同步API Key到后端失败:', error);
        }
    };
    ...
    const handleApiKeyUpdate = async (apiKey: string) => {
        ...
        // 同步到后端
        if (apiKey) {
            await syncApiKeyToBackend(apiKey);
        } else {
            // 清除后端API Key
            try {
                await fetch('http://localhost:8000/api/clear-key', {
                    method: 'DELETE',
                });
                console.log('后端API Key已清除');
            } catch (error) {
                console.error('清除后端API Key失败:', error);
            }
        }
        
        console.log('API Key已更新:', apiKey ? '已设置' : '已清除');
    };
            ...
            <header className={`px-4 py-3 flex items-center justify-between ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b shadow-sm z-10`}>
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button 
                        ...
                        className={`p-2 rounded-full ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'} transition-colors relative`}
                        title="API Key 管理"
                    >
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center mb-2">
                                                        <div className={`text-xl mr-2 ${
                                                            ...
                                                            {!['python_expert', 'copywriting_expert', 'story_master', 'rewrite_master', 'xiaohongshu_expert', 'xiaohongshu_daily', 'crazy_thursday', 'deep_thinker', 'decision_expert', 'food_critic', 'debate_expert', 'ancient_style'].includes(agent.id) && '🤖'}
                                                        </div>
